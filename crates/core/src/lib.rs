...
pub fn load_text(path: &Path) -> Result<(String, bool, usize), String> {
...
pub fn neighbor_image(path: &Path, forward: bool) -> Option<PathBuf> {
...
pub fn neighbor_text(path: &Path, forward: bool) -> Option<PathBuf> {
    let parent = path.parent()?;
...
pub fn recompute_count(query: &str, text: &str) -> usize {
    if query.is_empty() { return 0; }
    let q = query.to_ascii_lowercase();
    text.to_ascii_lowercase().matches(&q).count()
}

pub fn find_target_line(text: &str, query: &str, target_idx: usize) -> Option<usize> {
